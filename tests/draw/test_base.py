...
def test_ColorMap():
    ...
    for g in good_span:
        ...
        cm = topiary.draw.base.ColorMap(span=g)
    ...
    for b in bad_span:
        ...
        with pytest.raises(ValueError):
            cm = topiary.draw.base.ColorMap(span=b)
    ...
    for b in bad_channel:
        ...
        with pytest.raises(ValueError):
            cm = topiary.draw.base.ColorMap(R=b)
        with pytest.raises(ValueError):
            cm = topiary.draw.base.ColorMap(G=b)
        with pytest.raises(ValueError):
            cm = topiary.draw.base.ColorMap(B=b)

    # Test individual channels, up and down
    cm = topiary.draw.base.ColorMap(R=(0,1),G=(0,0),B=(0,0))
    assert cm.hex(0) == expected_results[(0,0,0)]
    assert cm.hex(1) == expected_results[(255,0,0)]

    cm = topiary.draw.base.ColorMap(R=(1,0),G=(0,0),B=(0,0))
    assert cm.hex(0) == expected_results[(255,0,0)]
    assert cm.hex(1) == expected_results[(0,0,0)]

    cm = topiary.draw.base.ColorMap(R=(0,0),G=(0,1),B=(0,0))
    assert cm.hex(0) == expected_results[(0,0,0)]
    assert cm.hex(1) == expected_results[(0,255,0)]

    cm = topiary.draw.base.ColorMap(R=(0,0),G=(1,0),B=(0,0))
    assert cm.hex(0) == expected_results[(0,255,0)]
    assert cm.hex(1) == expected_results[(0,0,0)]

    cm = topiary.draw.base.ColorMap(R=(0,0),G=(0,0),B=(0,1))
    assert cm.hex(0) == expected_results[(0,0,0)]
    ...
    cm = topiary.draw.base.ColorMap(R=(0,1),G=(0,1),B=(0,0))
    assert cm.hex(0) == expected_results[(0,0,0)]
    assert cm.hex(1) == expected_results[(255,255,0)]

    cm = topiary.draw.base.ColorMap(R=(0,1),G=(0,0),B=(0,1))
    assert cm.hex(0) == expected_results[(0,0,0)]
    assert cm.hex(1) == expected_results[(255,0,255)]

    cm = topiary.draw.base.ColorMap(R=(0,0),G=(0,1),B=(0,1))
    assert cm.hex(0) == expected_results[(0,0,0)]
    assert cm.hex(1) == expected_results[(0,255,255)]

    cm = topiary.draw.base.ColorMap(R=(0,1),G=(0,1),B=(0,1))
    assert cm.hex(0) == expected_results[(0,0,0)]
