__description__ = \
"""
Function to merge a set of blast dataframes.
"""
...
def merge_blast_df(blast_df_list):
    """
    Merge a list of blast output dataframes from multiple queries. Merge
    happens based on accession. If different queries returned an overlapping
    sequence region from the same accession, merge them together. The merge
    can include an arbitrary number of sequence. This merge will:
    ...
    Parameters
    ----------
        blast_df_list: list of dataframes returned by a blast call with
    ...
    try:
        blast_df_list = _arg_processors.process_iter(blast_df_list,
                                                     "blast_df_list")
    except ValueError:
