...
def make_blast_db(input_files,db_name,overwrite=False,makeblastdb_binary="makeblastdb"):
    ...
    Parameters
    ----------
        input_files: list of input files. Must be fasta files (with extension
                     .faa) or zipped fasta files (with extension .faa.gz).
        db_name: name of final database
        overwrite: whether or not to overwrite an existing database
        makeblastdb_binary: makeblastdb binary to use

    Return
    ------
        None. Creates a blast database with name db_name.
    """
    ...
    overwrite = _arg_processors.process_bool(overwrite)
