...
def test__prepare_for_blast(test_dataframes):
    ...
    default_kwargs = get_public_param_defaults(recip.recip_blast,
                                               recip._prepare_for_blast)
    ...
    with pytest.raises(ValueError):
        out = recip._prepare_for_blast(df,paralog_patterns,**kwargs)
    ...
    out = recip._prepare_for_blast(**kwargs)
    ...
    # ends up the same
    out = recip._prepare_for_blast(paralog_patterns={"stupid":"1"},**kwargs)
    patterns1 = out[2]
    out = recip._prepare_for_blast(paralog_patterns={"stupid":["1"]},**kwargs)
    patterns2 = out[2]
    out = recip._prepare_for_blast(paralog_patterns={"stupid":re.compile("1")},**kwargs)
    patterns3 = out[2]
    out = recip._prepare_for_blast(paralog_patterns={"stupid":[re.compile("1")]},**kwargs)
    patterns4 = out[2]
    ...
    for g in good_paralog_patterns:
        out = recip._prepare_for_blast(paralog_patterns=g,**kwargs)
    ...
    for b in bad_paralog_patterns:
        ...
        with pytest.raises(ValueError):
            out = recip._prepare_for_blast(paralog_patterns=b,**kwargs)
    ...
    for g in good_trues:
        out = recip._prepare_for_blast(ignorecase=g,**kwargs)
    ...
    for b in bad_ignorecase:
        ...
        with pytest.raises(ValueError):
            out = recip._prepare_for_blast(ignorecase=b,**kwargs)
    ...
    for g in good_args:
        out = recip._prepare_for_blast(max_del_best=g,**kwargs)
        assert out[3] == g
    ...
    for b in bad_args:
        ...
        with pytest.raises(ValueError):
            out = recip._prepare_for_blast(max_del_best=b,**kwargs)
    ...
    for g in good_args:
        out = recip._prepare_for_blast(min_call_prob=g,**kwargs)
        assert out[4] == g
    ...
    for b in bad_args:
        ...
        with pytest.raises(ValueError):
            out = recip._prepare_for_blast(min_call_prob=b,**kwargs)
    ...
    good_trues = [True,1,"1",np.ones(1,dtype=bool)[0]]
    for g in good_trues:
        out = recip._prepare_for_blast(use_start_end=g,**kwargs)
    ...
    for b in bad_use_start_end:
        ...
        with pytest.raises(ValueError):
            out = recip._prepare_for_blast(use_start_end=b,**kwargs)
    ...
    out = recip._prepare_for_blast(use_start_end=True,**kwargs)
    sequence_list = out[1]
    ...
    out = recip._prepare_for_blast(use_start_end=False,**kwargs)
    sequence_list = out[1]
    assert np.array_equal(np.array([len(s) for s in sequence_list]),
...
def test__make_recip_blast_calls(test_dataframes,recip_blast_hit_dfs):
    ...
    prep_kwargs = get_public_param_defaults(recip.recip_blast,
                                            recip._prepare_for_blast)
    ...
    prep_out = recip._prepare_for_blast(df,paralog_patterns,**prep_kwargs)
    ...
    for blast_type in recip_blast_hit_dfs:
        ...
        if blast_type.startswith("ncbi"):
            kwargs["ncbi_blast_db"] = "nr"
        out_df = recip._make_recip_blast_calls(hit_dfs=hit_dfs,**kwargs)
