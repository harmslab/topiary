...
import topiary
from topiary._arg_processors import process_paralog_patterns

import numpy as np
import pandas as pd
import re

def test_process_paralog_patterns():

    # Send in bad paralog_patterns data types (should be dict)
    bad_inputs = [1,-1,1.5,False,pd.DataFrame]
    for b in bad_inputs:
        with pytest.raises(ValueError):
            patterns = process_paralog_patterns(paralog_patterns=b)
    ...
    patterns = process_paralog_patterns(paralog_patterns={"test":"this"})
    ...
    patterns = process_paralog_patterns(paralog_patterns={"test":["this","other"]})
    assert len(patterns) == 1
    assert patterns[0][0].search("string matches this") is not None
    ...
    patterns = process_paralog_patterns(paralog_patterns={"test":(re.compile("this"),"other")})
    assert len(patterns) == 1
    assert patterns[0][0].search("string matches this") is not None
    assert patterns[0][0].search("string matches other") is not None
    assert patterns[0][0].search("string does not match") is None
