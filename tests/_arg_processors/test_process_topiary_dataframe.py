...
import topiary
from topiary._arg_processors import process_topiary_dataframe
...
def test_check_topiary_dataframe(test_dataframes):
    ...
    df = process_topiary_dataframe(test_dataframes["good-df"])
    ...
    for b in bad_inputs:
        with pytest.raises(ValueError):
            process_topiary_dataframe(b)

    # Make sure it drops empty lines
    df = process_topiary_dataframe(test_dataframes["good-test_blank-lines"])
    assert len(df) == 5
    ...
    df = process_topiary_dataframe(test_dataframes["good-test-keep-parse_number"])
    assert df.keep.dtypes is np.dtype(bool)
    assert np.array_equal(df.keep,np.array([True,False,
                                            True,False]))
    ...
    df = process_topiary_dataframe(test_dataframes["no-uid"])
    assert len(np.unique(df.uid)) == len(df)

    # Check replace bad uid.
    df = process_topiary_dataframe(test_dataframes["bad-uid"])
    for d in df.uid:
        assert type(d) is str
        assert len(d) == 10

    # Check make uid unique.
    df = process_topiary_dataframe(test_dataframes["duplicate-uid"])
    assert len(np.unique(df.uid)) == len(df)
    ...
    for k in bad_ott_keys:
        with pytest.raises(ValueError):
            df = process_topiary_dataframe(test_dataframes[k])

    # Check alignment
    df = process_topiary_dataframe(test_dataframes["good-df_with-good-alignment"])
    df["alignment"]
    ...
    checked_df = process_topiary_dataframe(input_df)
