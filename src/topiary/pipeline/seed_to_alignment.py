...
def seed_to_alignment(seed_df,
    ...
    # We are actually splitting the initial dataframe into three potential 
    # restart points: 
    # 01: BLAST XMLs
    # 02: Downloaded sequences
    # 03: The final dataframe (with OTT)
    #
    # To handle this, we'll manually increment step_counter and update 
    # expected_output for each part of the process. 
    
    # Check for 03_initial-dataframe.csv
    step_counter = 3
    expected_output = f"{step_counter:02d}_initial-dataframe.csv"
    run_calc = _check_restart(expected_output,restart)
    ...
    if run_calc:
        ...
        # Define names for intermediate restart points
        xml_prefix = "01_topiary-initial-blast"
        intermediate_file = "02_downloaded-sequences.csv"

        # If restart, look for existing blast xml files or intermediate hits
        if restart:

            import glob
            
            # Check for intermediate hits first. 
            if os.path.exists(intermediate_file):
                print(f"Found existing downloaded sequences ({intermediate_file}).")
                print("Using these instead of re-blasting/re-downloading.")
                blast_xml = None
                ncbi_blast_db = None
                local_blast_db = None
            else:
                # Look for new style prefixed names
                existing_xml = glob.glob(f"{xml_prefix}*.xml")

                # Look for old style random names
                existing_xml.extend(glob.glob("topiary-tmp_*_blast-out.xml"))
                existing_xml.extend(glob.glob("*_ncbi-blast-result.xml"))

                if len(existing_xml) > 0:
                    print(f"Found {len(existing_xml)} existing BLAST XML files. Using these instead of re-blasting.")
                    blast_xml = existing_xml
                    ncbi_blast_db = None
                    local_blast_db = None
        ...
        kwargs = {"seed_df":seed_df,
                  ...
                  "keep_blast_xml":True,
                  "name_prefix":xml_prefix,
                  "intermediate_file":intermediate_file,
                  "blast_taxid":blast_taxid}
